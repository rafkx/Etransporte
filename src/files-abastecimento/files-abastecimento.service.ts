...
export class FilesAbastecimentoService {
  constructor(
    @InjectRepository(FilesAbastecimento)
    private readonly repository: Repository<FilesAbastecimento>,
  ) {}
  ...
  async salvarDados(
    ...
    try {
      ...
      arquivo.url = `${req.protocol}://${req.get('host')}/files/abastecimento/${
        file.filename
      }`;
  ...
  download(fileName: string): StreamableFile {
    const file = fs.createReadStream(
      join(process.cwd(), `./files/abastecimento/${fileName}`),
    );
  ...
  findAll(id: string): Promise<FilesAbastecimento[]> {
    ...
    queryBuilder.leftJoinAndSelect(
      'files_abastecimento.abastecimento',
      'abastecimento',
    );
  ...
  async remove(fileName: string) {
    ...
    const queryBuilder = this.repository
      .createQueryBuilder('files_abastecimento')
      .delete()
