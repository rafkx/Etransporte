...
export class ServicoService {
  constructor(
    @InjectRepository(Servico) private readonly repository: Repository<Servico>,
  ) {}
  ...
  async findServico(
    ...
    if (text) {
      queryBuilder
        .where('servico.descricao LIKE :text', { text: `%${text}%` })
  ...
  async findServicoByFuncionario(
    id: string,
    pageOptionsDto: PageOptionsDto,
  ): Promise<PageDto<Servico>> {
    const queryBuilder = this.repository.createQueryBuilder('servico');
