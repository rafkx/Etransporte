...
export class FilesQuilometroService {
  constructor(
    @InjectRepository(FilesQuilometro)
    private readonly repository: Repository<FilesQuilometro>,
  ) {}
  ...
  async salvarDados(
    ...
    try {
      ...
      arquivo.url = `${req.protocol}://${req.get('host')}/files-quilometro/${
        file.filename
      }`;
  ...
  download(fileName: string): StreamableFile {
    const file = fs.createReadStream(
      join(process.cwd(), `./files/quilometro/${fileName}`),
    );
  ...
  async remove(fileName: string) {
    ...
    const queryBuilder = this.repository
      .createQueryBuilder('files_quilometro')
      .delete()
