...
export class FilesVeiculoService {
  constructor(@InjectRepository(FilesVeiculo) private readonly repository: Repository<FilesVeiculo>) { }
  ...
  download(fileName: string): StreamableFile {
    const file = fs.createReadStream(join(process.cwd(), `./files/veiculo/${fileName}`));
