...
export class FilesFuncionarioService {
  constructor(@InjectRepository(FilesFuncionario) private readonly repository: Repository<FilesFuncionario>) { }
  ...
  async salvarDados(
    ...
    req: Request
      ...
      try {
          ...
          arquivo.url = `${req.protocol}://${req.get('host')}/files-funcionario/${file.filename}`;
  ...
  download(fileName: string): StreamableFile {
    const file = fs.createReadStream(join(process.cwd(), `./files/funcionario/${fileName}`));
    return new StreamableFile(file);
  }

  findAll(id: string): Promise<FilesFuncionario[]> {
    const queryBuilder = this.repository.createQueryBuilder('files_funcionario');
    queryBuilder.leftJoinAndSelect('files_funcionario.funcionario', 'funcionario');
