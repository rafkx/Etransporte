...
export class FilesServicoService {
  constructor(
    @InjectRepository(FilesServico)
    private readonly repository: Repository<FilesServico>,
  ) {}
  ...
  download(fileName: string): StreamableFile {
    const file = fs.createReadStream(
      join(process.cwd(), `./files/servico/${fileName}`),
    );
  ...
  async remove(fileName: string) {
    ...
    const queryBuilder = this.repository
      .createQueryBuilder('files_servico')
      .delete()
      .from('files_servico')
